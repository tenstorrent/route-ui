...
const OperationGroupRender: FC<OperationGroupRenderProps> = ({ node }) => {
    const selectedGroup = useSelector(getOperand(node.operation?.name ?? ''));
    const showOperationNames = useSelector(getShowOperationNames);
    const color = getGroupColor(node.operation?.name ?? '');
    const shouldShowBorder =
        node.operation?.name !== '' && selectedGroup && (selectedGroup?.selected || showOperationNames);
